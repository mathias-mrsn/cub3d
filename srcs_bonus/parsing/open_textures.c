/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   open_textures.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mamaurai <mamaurai@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/27 18:36:07 by mamaurai          #+#    #+#             */
/*   Updated: 2022/03/23 13:11:02 by mamaurai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	open_door_texture(t_cub *s)
{	
	__memset(&s->textures->door, 0, sizeof(t_img));
	s->textures->walls[DOOR].ptr = mlx_xpm_file_to_image(s->mlx,
			s->parser->walls_path[DOOR],
			&s->textures->walls[DOOR].x, &s->textures->walls[DOOR].y);
	if (NULL == s->textures->walls[DOOR].ptr)
		__unable_to_open_xpm__(__LINE__, __FILE__);
	s->textures->walls[DOOR].addr
		= mlx_get_data_addr(s->textures->walls[DOOR].ptr,
			&s->textures->walls[DOOR].bpp, &s->textures->walls[DOOR].size_line,
			&s->textures->walls[DOOR].endian);
	if (NULL == s->textures->walls[DOOR].addr)
		__unable_to_open_xpm__(__LINE__, __FILE__);
}

void	open_sprite_texture(t_cub *s)
{
	uint32_t	idx;

	idx = 0;
	s->textures->sprite = __malloc(sizeof(t_img) * \
			s->parser->nbr_sprite, PARSER_STACK);
	while (idx < s->parser->nbr_sprite)
	{
		s->textures->sprite[idx].ptr = mlx_xpm_file_to_image(s->mlx,
				s->parser->sprite_path[idx], &s->textures->sprite[idx].x,
				&s->textures->sprite[idx].y);
		if (NULL == s->textures->sprite[idx].ptr)
			__unable_to_open_xpm__(__LINE__, __FILE__);
		s->textures->sprite[idx].addr
			= mlx_get_data_addr(s->textures->sprite[idx].ptr,
				&s->textures->sprite[idx].bpp,
				&s->textures->sprite[idx].size_line,
				&s->textures->sprite[idx].endian);
		if (NULL == s->textures->sprite[idx].addr)
			__unable_to_open_xpm__(__LINE__, __FILE__);
		idx++;
	}
}

void
	open_textures(t_cub *s)
{
	uint8_t	idx;

	idx = 0;
	while (idx < NBR_WALL_SIDES - 1)
	{
		__memset(&s->textures->walls[idx], 0, sizeof(t_img));
		s->textures->walls[idx].ptr = mlx_xpm_file_to_image(s->mlx,
				s->parser->walls_path[idx], &s->textures->walls[idx].x,
				&s->textures->walls[idx].y);
		if (NULL == s->textures->walls[idx].ptr)
			__unable_to_open_xpm__(__LINE__, __FILE__);
		s->textures->walls[idx].addr
			= mlx_get_data_addr(s->textures->walls[idx].ptr,
				&s->textures->walls[idx].bpp,
				&s->textures->walls[idx].size_line,
				&s->textures->walls[idx].endian);
		if (NULL == s->textures->walls[idx].addr)
			__unable_to_open_xpm__(__LINE__, __FILE__);
		idx++;
	}
	s->textures->ceil = s->parser->ceil;
	s->textures->floor = s->parser->floor;
	open_door_texture(s);
	open_sprite_texture(s);
}
